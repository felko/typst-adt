...
#import "../../src/methods/common.typ": (
  project-constr, project-constr-args, result-trace-constr,
)
...
#assert.eq(adt.validate(adt.TRACE, args-arity-error.trace), ok(
  args-arity-error.trace,
))
#assert(
  adt.result-error-to-string(args-arity-error).contains("expected call shape"),
)
...
#assert.eq(adt.validate(adt.TRACE, args-extra-error.trace), ok(
  args-extra-error.trace,
))
#assert(
  adt
    .result-error-to-string(args-extra-error)
    .contains("unexpected named arguments"),
)

#let args-missing-error = adt.validate-args(named-dom)
#assert.eq(args-missing-error.trace.__tag__, "trace/args-missing-named")
#assert.eq(adt.validate(adt.TRACE, args-missing-error.trace), ok(
  args-missing-error.trace,
))
#assert(
  adt
    .result-error-to-string(args-missing-error)
    .contains("missing named arguments"),
)
...
#assert.eq(adt.validate(adt.TRACE, constr-missing-error.trace), ok(
  constr-missing-error.trace,
))
#assert(
  adt.result-error-to-string(constr-missing-error).contains("missing argument"),
)
...
#assert.eq(adt.validate(adt.TRACE, constr-extra-error.trace), ok(
  constr-extra-error.trace,
))
#assert(
  adt.result-error-to-string(constr-extra-error).contains("extra arguments"),
)
...
#assert.eq(adt.validate(adt.TRACE, projected-missing-error.trace), ok(
  projected-missing-error.trace,
))
#assert(
  adt.result-error-to-string(projected-missing-error).contains("missing field"),
)
...
#assert.eq(adt.validate(adt.TRACE, projected-extra-error.trace), ok(
  projected-extra-error.trace,
))
...
#assert.eq(adt.validate(adt.TRACE, named-constr-error.trace), ok(
  named-constr-error.trace,
))
#assert(
  adt
    .result-error-to-string(named-constr-error)
    .contains("constructor `tree-leaf`"),
)
