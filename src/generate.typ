...
#let validate-runtime-constr(validate-runtime, constr-spec, value) = constr-spec-elim(
  none_: ok(value),
  fields: field-specs => {
...
#let validate-runtime(spec, value) = spec-elim(
  ...
  any: () => ok(value),
  union_case: (name, elems) => result-map(_ => value, result-any(elem => validate-runtime(elem, value), elems)),
  enum: (name, constrs) => {
  ...
  struct: (name, fields) => validate-runtime-constr(validate-runtime, (__tag__: "constr-spec/fields", fields: fields), value),
  array_case: (name, inner) => result-map(_ => value, validate(spec, value)),
  dictionary_case: (name, key, inner) => result-map(_ => value, validate(spec, value)),
  function_case: (name, dom, cod) => result-map(_ => value, validate(spec, value)),
...
#let generate-constr(tag, constr-spec) = constr-spec-elim(
  ...
  fields: _ => {
    if tag == none {
      (.. args) => attach-ops(auto, result-unwrap(validate-constr(constr-spec, .. args)))
    } else {
...
#let generate-constr-with-spec(spec, tag, constr-spec, ops: (:)) = constr-spec-elim(
...
#let generate-value-intro(spec, ops: (:)) = value => attach-ops(spec, result-unwrap(validate(spec, value)), ops: ops)
...
#let generate-enum-intros(spec, constrs, ops: (:)) = constrs.pairs().map(((constr-name, constr-spec)) => {
  (constr-name, generate-constr-with-spec(spec, constr-name, constr-spec, ops: ops))
}).to-dict()
...
#let generate-intro(spec, ops: (:)) = spec-elim(
  ...
  struct: (name, fields) => (
    intro: generate-constr-with-spec(spec, none, (__tag__: "constr-spec/fields", fields: fields), ops: ops)
  ),
  enum: (name, constrs) => {
  ...
  dictionary_case: (name, key, inner) => (intro: generate-value-intro(spec, ops: ops)),
...
#let generate-enum-annotate(spec, constrs, ops: (:)) = (annotate: (.. args) => {
  assert(
    args.pos().len() == 0,
    message: "expected no positional arguments, got " + repr(args.pos())
  )
  let named = args.named()
  assert(named.keys().contains("__ann__"), message: "missing `__ann__` annotation spec")
  let ann-specs = named.remove("__ann__")
  assert(type(ann-specs) == dictionary, message: "expected `__ann__` to be a dictionary")
      ...
      for (field-name, field-spec) in constr-spec.fields.pairs() {
        ...
        if spec-is-fix(field-spec) {
          ...
          algebra-fields.insert(
            field-name,
            ann-specs.keys().map(ann-name => (ann-name, annotated-child.at(ann-name))).to-dict()
          )
        } else {
